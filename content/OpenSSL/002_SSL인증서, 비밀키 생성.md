---
title: SSL 인증서와 비밀키 생성 과정
tags: [open_ssl, ssl, certificate, private_key, csr, ca]
draft: false
---
...
```mermaid
flowchart TD
    A["1. 비밀키 생성<br/>openssl genrsa"] --> B["2. 공개키 추출 (선택)<br/>openssl rsa -pubout"]
    A --> C["3. CSR 생성<br/>비밀키로 디지털 서명"]
    C --> D{인증서 발급 방식}
    D -->|공식 CA| E[CA에 CSR 제출]
    D -->|자체 서명| F[자체 비밀키로 서명]
    E --> G["CA가 서명 검증 후<br/>인증서(crt) 발급"]
    F --> H["자체 서명 인증서 생성<br/>openssl x509"]
```
